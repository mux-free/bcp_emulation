...
c     Check whether a tracing file exists; if yes, read it
  ...
  10         continue
             ntrace = ntrace + 1
             read(10,*,end=25) tvar(ntrace), tfac(ntrace), tfil(ntrace)
             goto 10
  25         continue
             ntrace = ntrace -1            
         close(10)
...
c     Now check that all files are available
     ...
     >                                  //'/'//trim(mm)//'/'
         ...
         print*,' ERROR: file is missing : ',prefix//trim(datestr)
         stop
  40     continue       
      enddo
...
c       Loop over all trajectories - the update loop goes from 0 to (timeinc/ts-1),
c       but the last time step needs also to be saved: therefore, to (timeinc/ts-1)+1
        nactive   = 0
        file_load = 0
